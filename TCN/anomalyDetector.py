...
# 这段代码定义了一个函数 fit_norm_distribution_param，其目的是拟合一组预测误差的正态分布参数（均值和协方差矩阵）。
# 函数使用了一个训练好的模型对数据进行预测，并计算预测值与真实值之间的误差，然后基于这些误差计算正态分布的参数（均值和协方差矩阵）。
def fit_norm_distribution_param(args, model, train_dataset, channel_idx=0):
    # args: 包含多个参数的结构，比如预测窗口大小（prediction_window_size）和设备信息（如 CPU 或 GPU）。
    # model: 一个训练好的神经网络模型，用于对数据进行预测。
    # train_dataset: 训练数据集。
    # channel_idx: 需要使用的通道索引，默认为 0。

    predictions = []    #存储模型的预测值
